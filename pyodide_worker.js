...
// Message types
const MESSAGE_TYPES = {
    REQUEST: "REQUEST",       // Main script sends a request
    RESPONSE: "RESPONSE",     // Worker sends a response
    INFO_REQUEST: "INFO_REQUEST", // Worker asks for more info
...
self.onmessage = async (event) => {
    const { id, type, context } = event.data;
    workerId = id;
    ...
    if (type === MESSAGE_TYPES.REQUEST) {
        ...
        await micropip.install("python_tsp");
        await pyodide.loadPackagesFromImports(python);
        const dict = pyodide.globals.get("dict");
        const globals = dict(Object.entries(context));
        try {
            const result = await pyodide.runPythonAsync(python, { globals });
            self.postMessage({ id, type: MESSAGE_TYPES.RESPONSE, result });
        } catch (error) {
            console.log(error);
            self.postMessage({ id, type: MESSAGE_TYPES.RESPONSE, error: error.message });
        }
    }

    if (type === MESSAGE_TYPES.INFO_RESPONSE) {
        // Got the info, continue processing
        commandResolve(context);
    }
};

self.javascriptInterface = function (command, context) {
...
from js import javascriptInterface

console.log("ROUTING: Commencing Task")
...
async def get_route(start, end):
    api_time = await javascriptInterface("route", dumps({"start": {"lon": float(start.lon), "lat": float(start.lat)}, "end": {"lon": float(end.lon), "lat": float(end.lat)}}))
    assert api_time > 0
    return api_time
...
# greedy DFS search; guarantees to return node counts for a Hamiltonian path
def find_diversions(graph):
    visits = {n: 0 for n in list(graph)}
    backtrace = []
...
async def main():
    ...
    if not poi.empty:
        ...
        console.log("ROUTING: Grouping POI by nearest home")
        ...
        return_poi = return_poi.join(homes.set_index("name"), on="center", rsuffix="_center")
        return_poi = return_poi[["lat", "lon", "lat_center", "lon_center"]].reset_index(drop=True)
        success = (await javascriptInterface("cache_poi_assoc", dumps(return_poi.to_dict("records")))).toPy()
        assert success > 0

    # remove stations with >= xx minute distance using API if not favorite
    console.log("ROUTING: Building network")
    ...
    # update Graph weights using API
    for edge in G.edges:
    ...
    if SLOW_ALGORITHM:
        console.log("ROUTING: drafting daygraphs. Please be patient")
        # add center
        ...
        console.log("ROUTING: Determining core groups")
        core_groups, G, removed_edges = get_core_groups(G, center, stations)
        console.log("ROUTING core groups:", dumps([list(c) for c in core_groups]))

        console.log("ROUTING: Associating remaining groups")
        ...
        console.log("ROUTING: Drafting day graphs")
        ...
        console.log("ROUTING final groups:", dumps([list(c) for c in core_groups]))


    console.log("ROUTING: Optimizing daily routes")
    optimal_routes = []
    for d in range(len(day_graphs)):
        console.log("ROUTING Day", d+1)
        ...
        while True:
            ...
            # reconnect graph
            parts = list(nx.connected_components(day_graph.subgraph(include_nodes)))
            if len(parts) > 1:
                for i in range(len(parts)):
                    for other_part in parts[i+1:]:
                        day_graph.add_edges_from(fetch_additional_routes(other_part, parts[i]))

        # rotate optimal route
        optimal_route = left_rotate_array(optimal_route, len(
            optimal_route), optimal_route.index(center["name"]))[1:]
        route_coordinates = [{"lon": float(stations.loc[stations["name"]==poi, "lon"].iloc[0]), "lat": float(stations.loc[stations["name"]==poi, "lat"].iloc[0])} for poi in optimal_route]
        optimal_routes.append({"route": route_coordinates, "duration": optimal_distance})
